...
# No flight is delayed more than 2 hours is false.
```
...
sfo_feb_flights %>% group_by(carrier) %>% summarize(median = median(arr_delay), iqr = IQR(arr_delay))
...
nycflights %>% group_by(month) %>% summarize(mean_dd = mean(dep_delay)) %>% arrange(desc(mean_dd))
# July
```
...
ggplot(nycflights, aes(x = avg_speed, y = distance)) + geom_line() + geom_point()
...
nycflights %>% summarize(depd_arrot = sum(dep_type == "delayed" && arr_type == "on time") / sum(dep_type == "delayed"))
```